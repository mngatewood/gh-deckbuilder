...
describe("Selected Cards Reducer", () => {
  ...
  it("should add a selected card to state", () => {
    ...
    expect(selectedCardsReducer(undefined,
      actions.addSelectedCard(mocks.mockCard))).toEqual(expected);
  });
  ...
  it("should add multiple selected cards to state", () => {
    ...
    expect(selectedCardsReducer(undefined,
      actions.addSelectedCards(mocks.mockCards))).toEqual(expected);
  });
  ...
  it("should remove selected card from state", () => {
    ...
    expect(selectedCardsReducer(mocks.mockCards,
      actions.removeSelectedCard(toDelete))).toEqual(expected);
  });
  ...
  it("should remove all selected cards from state", () => {
    ...
    expect(selectedCardsReducer(mocks.mockCards,
      actions.removeSelectedCards(mocks.mockCards))).toEqual(expected);
  });
});
